...
const [emloyeename, setemloyeename] = useState("");
  ...
  const getAlldepartments = async () => {
    try {
      const response = await axios.get(`http://192.168.1.5:8000/alldepartments`);
  ...
  const attritionemloyee = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.1.5:5000/Addemployee', { //send data to api
        Age: age,
...
setresultattr(response.data.result) //result is save
    ...
    const performSearch = async () => {
      ...
      try {
        const response = await axios.get(
          `http://192.168.1.5:8000/api/searchlist?EmployeeNumber=${filters.employeeID}&Age=${filters.age}&JobRole=${filters.position}&Gender=${filters.gender}&Department=${filters.department}`
        );
    ...
    const getAllemployee = async () => {
        try {
            const response = await axios.get(`http://192.168.1.5:8000/allemployees`);
    ...
    return (
        <SafeAreaView style={styles.container}>
            <ScrollView>
                ...
                <TouchableOpacity style={styles.retentionButton} onPress={handleRetention}>
                    <Text style={styles.retentionButtonText}>Retention Strategies</Text>
                </TouchableOpacity>
